                ...
                pendentesMock.map(item => (
                    <div key={item.id} className="bg-card-primary border border-white/5 rounded-3xl p-6 flex flex-col md:flex-row justify-between md:items-center gap-6">
                        ...
                        <div className="flex items-center gap-2 w-full md:w-auto">
                            <button onClick={() => alert("Evento homologado e adicionado ao calendário!")} className="flex-1 md:flex-none h-9 px-4 bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1"><FiCheck/> Aprovar</button>
                            <button onClick={() => alert("Solicitação de evento descartada.")} className="flex-1 md:flex-none h-9 px-4 bg-slate-800 hover:bg-slate-700 text-slate-300 font-bold text-xs rounded-xl transition-all cursor-pointer flex items-center justify-center gap-1"><FiX/> Recusar</button>
                        </div>
                    </div>
                ))
            ) : (
